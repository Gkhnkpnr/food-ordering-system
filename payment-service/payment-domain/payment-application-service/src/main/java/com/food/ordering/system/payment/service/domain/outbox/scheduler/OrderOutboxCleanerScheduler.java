...
public class OrderOutboxCleanerScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        Optional<List<OrderOutboxMessage>> orderOutboxMessageResponses =
                orderOutboxHelper.getOrderOutboxMessageByOutboxStatus(OutboxStatus.COMPLETED);
        if (orderOutboxMessageResponses.isPresent() && !orderOutboxMessageResponses.get().isEmpty()) {
            List<OrderOutboxMessage> orderOutboxMessages = orderOutboxMessageResponses.get();
